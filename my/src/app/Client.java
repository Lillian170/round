...
public class Client extends JFrame {
	...
	public Client() {
		...
		result = new JLabel(); // 創建一個用於顯示結果的標籤
		submitButton = new JButton("送出"); // 創建一個標籤為"送出"的按鈕
		setLayout(new FlowLayout()); // 使用流式布局
		add(new JLabel("帳號：")); // 添加一個標籤，顯示"帳號："
		add(usernameField); // 添加使用者名稱文本框
		add(new JLabel("密碼：")); // 添加一個標籤，顯示"密碼："
		add(passwordField); // 添加密碼框
		add(result); // 添加結果標籤
		add(submitButton); // 添加提交按鈕
		setDefaultCloseOperation(EXIT_ON_CLOSE); // 設定關閉窗口時的默認操作
		setSize(300, 150); // 設定窗口大小為300x150
		setVisible(true); // 顯示窗口
		submitButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (sendData()) {
					setSize(640, 480); // 如果數據發送成功，則將窗口大小設定為640x480
				}
			}
		});
		try {
